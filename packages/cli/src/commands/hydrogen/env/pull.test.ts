  ...
  it('warns if there are any variables marked as secret', async () => {
    ...
    await inTemporaryDirectory(async (tmpDir) => {
      ...
      expect(outputMock.warn()).toStrictEqual(
        'Your Hydrogen storefront contains environment variables marked as ' +
          'secret and their values were not pulled.',
      );
    });
  });
  ...
  describe('when there is no linked storefront', () => {
    ...
    it('renders an error message', async () => {
      await inTemporaryDirectory(async (tmpDir) => {
        ...
        expect(outputMock.error()).toMatch(/No linked Hydrogen storefront\./);
      });
    });
  });
